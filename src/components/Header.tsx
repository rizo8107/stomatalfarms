    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/50">
      <div className="container flex items-center justify-between h-16 md:h-20">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-1 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors duration-200 outline-none">
              Aurora
            ...
            <DropdownMenuContent align="start" className="w-56">
              {auroraCategories.map((category) => (
                <DropdownMenuItem key={category.name} asChild>
                  <Link to={category.href} className="hover:bg-[#8dcc5b]/10">
                    {category.name}
                  </Link>
                </DropdownMenuItem>
              ))}
        ...
        <div className="md:hidden absolute top-full left-0 right-0 bg-background border-b border-border">
          <nav className="container py-6 flex flex-col gap-4">
            ...
            <div className="border-b border-border pb-4">
              <p className="text-xs uppercase tracking-wider text-muted-foreground mb-3 font-medium">Aurora</p>
              {auroraCategories.map((category) => (
                <Link
                  key={category.name}
                  to={category.href}
                  className="block text-base font-medium text-foreground py-2 pl-4 hover:text-[#8dcc5b] transition-colors"
            ...
            {navLinks.map((link) => (
              link.href.startsWith('/') ? (
                <Link
                  key={link.name}
                  to={link.href}
                  className="text-base font-medium text-foreground py-2 hover:text-[#8dcc5b] transition-colors"
                  onClick={() => setIsMenuOpen(false)}
                >
                  {link.name}
                </Link>
